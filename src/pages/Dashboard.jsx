...
export default function Dashboard() {
  ...
  // Aggregate stats from live district data
  const totalPrimary      = districts.reduce((acc, d) => acc + (d.p_schools      || 0), 0)
  const totalSecondary    = districts.reduce((acc, d) => acc + (d.s_schools      || 0), 0)
  const totalTertiary     = districts.reduce((acc, d) => acc + (d.t_institutions || 0), 0)
  const totalSchools      = totalPrimary + totalSecondary + totalTertiary
  const totalDistricts    = districts.length
  const criticalDistricts = districts.filter(d => calcScore(d.p_age_pop, d.p_schools, 'primary') >= 80)

  // Total tracked school-age population across all districts (primary age-group)
  const totalAgePop       = districts.reduce((acc, d) => acc + (d.p_age_pop || 0), 0)
  // Estimated MoE target: 1 school per 400 students, so coverage = schools / needed × 100
  const totalNeeded       = Math.ceil(totalAgePop / 400)
  const coveragePct       = totalNeeded > 0
    ? Math.min(100, Math.round((totalPrimary / totalNeeded) * 100))
    : 0
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          ...
          subValue={`P: ${totalPrimary.toLocaleString()} · S: ${totalSecondary.toLocaleString()} · T: ${totalTertiary.toLocaleString()}`}
          icon="🏫"
        />
        <StatCard
          title="Total Districts"
          value={totalDistricts}
          subValue={`${criticalDistricts.length} in Critical Need`}
          icon="🇲🇼"
        />
        <StatCard
          title="School Supply Rate"
          value={`${coveragePct}%`}
          subValue={`${totalPrimary.toLocaleString()} schools for ${totalNeeded.toLocaleString()} needed`}
          icon="👥"
        ...
        <StatCard
          ...
          subValue="Need Score ≥ 80 (Primary)"
          icon="📍"
          color="text-red-600"
        />
      </div>
          ...
          <div className="mt-6 flex items-center justify-between pt-4 border-t border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">
                {totalDistricts} Districts Loaded · {new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 bg-[#1a5276] rounded-full" />
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Supabase: Connected</span>
            </div>
          </div>
        </div>
      </div>
